...
def api_process_puter_image():
    ...
    try:
        ...
        if sender_number and public_url and not public_url.startswith('http://localhost'):
            # Lancer l'envoi WhatsApp en arrière-plan avec gestion d'erreur
            try:
                threading.Thread(
                ...
                logger.error(f"❌ Erreur création thread WhatsApp: {thread_error}")
    ...
    if incoming_msg:
        ...
        try:
            ...
            try:
                ...
                print(f"❌ Erreur envoi audio async: {audio_error}")
